...
for (region in brain_regions_pre) {
  X_Centiles_FEP_MRI_baseline_symptoms_raw_scores[[paste0(region,"2")]] <- X_Centiles_FEP_MRI_baseline_symptoms_raw_scores[[region]]
}

# Convertir centiles en z scores
for (region in brain_regions_pre) {
  X_Centiles_FEP_MRI_baseline_symptoms_raw_scores[[region]] <- scale(X_Centiles_FEP_MRI_baseline_symptoms_raw_scores[[region]])
}
...
for (var in variables) {
  ...
  # Store the ps of all variables for 902 comparisons (11x82)
  all_ps_BL_centils <- c(all_ps_BL_centils, p)
  ...
  # Store the ps of all variables for 902 comparisons
  ...
  # Store the ps of all variables for 902 comparisons
  all_ps_interaction <- c(all_ps_interaction, p)
}
  
  # SEPARATE CORTICAL AND SUBCORTICAL DATA FOR PLOTTING 82 COMPARISONS
  
  # Initialize empty vectors to store concatenated values
  results_list_BL_centils_cor <- list()
  results_list_time_cor <- list()
  results_list_CPZ_cor <- list()
  results_list_interaction_cor <- list()
  
  results_list_BL_centils_sub <- list()
  results_list_time_sub <- list()
  results_list_CPZ_sub <- list()
  results_list_interaction_sub <- list()
...
# *** Perform FDR correction on p-values for 902 comparisons (82 cortical regions x 11 symptoms)
p_fdr2_BL_centils <- p.adjust(all_ps_BL_centils, method = "fdr")
sig2_BL_centils <- c(p_fdr2_BL_centils < 0.05)
...
p_fdr2_interaction <- p.adjust(all_ps_interaction, method = "fdr")
sig2_interaction <- c(p_fdr2_interaction < 0.05)

# SEPARATE CORTICAL AND SUBCORTICAL DATA FOR PLOTTING 902 COMPARISONS

# Create a sequence from 1 to 902
full_indices <- 1:902
...
# Split the sequence into 11 segments and extract the first 68 rows of each segment
extracted_indices_cor <- lapply(split(full_indices, rep(1:11, each = 82)), function(x) x[indices_cor])
# Flatten the list of extracted indices to get a vector
extracted_indices_cor <- unlist(extracted_indices_cor)

# Subset original variables using the extracted indices and split them into 11 parts of 68 each
p_fdr_new_BL_centils_cor <- split(p_fdr2_BL_centils[extracted_indices_cor], rep(1:11, each = 68))
sig_new_BL_centils_cor <- split(sig2_BL_centils[extracted_indices_cor], rep(1:11, each = 68))
p_fdr_new_time_cor <- split(p_fdr2_time[extracted_indices_cor], rep(1:11, each = 68))
sig_new_time_cor <- split(sig2_time[extracted_indices_cor], rep(1:11, each = 68))
p_fdr_new_CPZ_cor <- split(p_fdr2_CPZ[extracted_indices_cor], rep(1:11, each = 68))
sig_new_CPZ_cor <- split(sig2_CPZ[extracted_indices_cor], rep(1:11, each = 68))
p_fdr_new_interaction_cor <- split(p_fdr2_interaction[extracted_indices_cor], rep(1:11, each = 68))
sig_new_interaction_cor <- split(sig2_interaction[extracted_indices_cor], rep(1:11, each = 68))

# Rename the rows of the resulting list with the desired names
names(p_fdr_new_BL_centils_cor) <- variables
names(sig_new_BL_centils_cor) <- variables
names(p_fdr_new_time_cor) <- variables
names(sig_new_time_cor) <- variables
names(p_fdr_new_CPZ_cor) <- variables
names(sig_new_CPZ_cor) <- variables
names(p_fdr_new_interaction_cor) <- variables
names(sig_new_interaction_cor) <- variables

# SUBCORTICAL

# Split the sequence into 11 segments and extract the first 68 rows of each segment
extracted_indices_sub <- lapply(split(full_indices, rep(1:11, each = 82)), function(x) x[indices_sub])
# Flatten the list of extracted indices to get a vector
extracted_indices_sub <- unlist(extracted_indices_sub)

# Subset original variables using the extracted indices and split them into 11 parts of 68 each
p_fdr_new_BL_centils_sub <- split(p_fdr2_BL_centils[extracted_indices_sub], rep(1:11, each = 14))
sig_new_BL_centils_sub <- split(sig2_BL_centils[extracted_indices_sub], rep(1:11, each = 14))
p_fdr_new_time_sub <- split(p_fdr2_time[extracted_indices_sub], rep(1:11, each = 14))
sig_new_time_sub <- split(sig2_time[extracted_indices_sub], rep(1:11, each = 14))
p_fdr_new_CPZ_sub <- split(p_fdr2_CPZ[extracted_indices_sub], rep(1:11, each = 14))
sig_new_CPZ_sub <- split(sig2_CPZ[extracted_indices_sub], rep(1:11, each = 14))
p_fdr_new_interaction_sub <- split(p_fdr2_interaction[extracted_indices_sub], rep(1:11, each = 14))
sig_new_interaction_sub <- split(sig2_interaction[extracted_indices_sub], rep(1:11, each = 14))

# Rename the rows of the resulting list with the desired names
names(p_fdr_new_BL_centils_sub) <- variables
names(sig_new_BL_centils_sub) <- variables
names(p_fdr_new_time_sub) <- variables
names(sig_new_time_sub) <- variables
names(p_fdr_new_CPZ_sub) <- variables
names(sig_new_CPZ_sub) <- variables
names(p_fdr_new_interaction_sub) <- variables
names(sig_new_interaction_sub) <- variables

# Make a copy of results_list
results_list2_BL_centils_cor <- results_list_BL_centils_cor
results_list2_time_cor <- results_list_time_cor
results_list2_CPZ_cor <- results_list_CPZ_cor
results_list2_interaction_cor <- results_list_interaction_cor

results_list2_BL_centils_sub <- results_list_BL_centils_sub
results_list2_time_sub <- results_list_time_sub
results_list2_CPZ_sub <- results_list_CPZ_sub
results_list2_interaction_sub <- results_list_interaction_sub

# Update results_list2 with the new significance results (for 902 comparisons)
for (var in variables) {
  # Replace the values in 'p_fdr' and 'sig' columns with the corresponding values from 'p_fdr_new' and 'sig_new'
  results_list2_BL_centils_cor[[var]][["p_fdr"]] <- p_fdr_new_BL_centils_cor[[var]]
  results_list2_BL_centils_cor[[var]][["sig"]] <- sig_new_BL_centils_cor[[var]]
  results_list2_time_cor[[var]][["p_fdr"]] <- p_fdr_new_time_cor[[var]]
  results_list2_time_cor[[var]][["sig"]] <- sig_new_time_cor[[var]]
  results_list2_CPZ_cor[[var]][["p_fdr"]] <- p_fdr_new_CPZ_cor[[var]]
  results_list2_CPZ_cor[[var]][["sig"]] <- sig_new_CPZ_cor[[var]]
  results_list2_interaction_cor[[var]][["p_fdr"]] <- p_fdr_new_interaction_cor[[var]]
  results_list2_interaction_cor[[var]][["sig"]] <- sig_new_interaction_cor[[var]]
  
  results_list2_BL_centils_sub[[var]][["p_fdr"]] <- p_fdr_new_BL_centils_sub[[var]]
  results_list2_BL_centils_sub[[var]][["sig"]] <- sig_new_BL_centils_sub[[var]]
  results_list2_time_sub[[var]][["p_fdr"]] <- p_fdr_new_time_sub[[var]]
  results_list2_time_sub[[var]][["sig"]] <- sig_new_time_sub[[var]]
  results_list2_CPZ_sub[[var]][["p_fdr"]] <- p_fdr_new_CPZ_sub[[var]]
  results_list2_CPZ_sub[[var]][["sig"]] <- sig_new_CPZ_sub[[var]]
  results_list2_interaction_sub[[var]][["p_fdr"]] <- p_fdr_new_interaction_sub[[var]]
  results_list2_interaction_sub[[var]][["sig"]] <- sig_new_interaction_sub[[var]]
}
...
plot <- regional_brainmap_representation_borders(results_list_BL_centils_cor[[var]], var, 10, -10, 0, "vertical") # FDR corrected for 82 comparisons

# Render the plot
print(plot)

# Save the plot to a specific folder as a PNG file
ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/BL_CENTILS_FDR_82_comparisons/",var,".png"), plot)

## TIME 82 comparisons

# Call the function to generate the plot
plot <- regional_brainmap_representation_borders(results_list_time_cor[[var]], var, 20, -20, 0, "vertical") # FDR corrected for 82 comparisons

# Render the plot
print(plot)

# Save the plot to a specific folder as a PNG file
ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/TIME_FDR_82_comparisons/",var,".png"), plot)

## CPZ 82 comparisons

# Call the function to generate the plot
plot <- regional_brainmap_representation_borders(results_list_CPZ_cor[[var]], var, 20, -20, 0, "vertical") # FDR corrected for 82 comparisons

# Render the plot
print(plot)

# Save the plot to a specific folder as a PNG file
ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/CPZ_FDR_82_comparisons/",var,".png"), plot)
...
# Save the plot to a specific folder as a PNG file
ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/INTERACTION_FDR_82_comparisons/",var,".png"), plot)

## BL CENTILS 902 comparisons

# Call the function to generate the plot
plot <- regional_brainmap_representation_borders(results_list2_BL_centils_cor[[var]], var, 10, -10, 0, "vertical") # FDR corrected for 902 comparisons

# Render the plot
print(plot)

# Save the plot to a specific folder as a PNG file
ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/BL_CENTILS_FDR_902_comparisons/",var,".png"), plot)

## TIME 902 comparisons

# Call the function to generate the plot
plot <- regional_brainmap_representation_borders(results_list2_time_cor[[var]], var, 20, -20, 0, "vertical") # FDR corrected for 902 comparisons

# Render the plot
print(plot)

# Save the plot to a specific folder as a PNG file
ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/TIME_FDR_902_comparisons/",var,".png"), plot)

## CPZ 902 comparisons

# Call the function to generate the plot
plot <- regional_brainmap_representation_borders(results_list2_CPZ_cor[[var]], var, 20, -20, 0, "vertical") # FDR corrected for 902 comparisons

# Render the plot
print(plot)

# Save the plot to a specific folder as a PNG file
ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/CPZ_FDR_902_comparisons/",var,".png"), plot)


## INTERACTION 902 comparisons

# Call the function to generate the plot
plot <- regional_brainmap_representation_borders(results_list2_interaction_cor[[var]], var, 15, -15, 0, "vertical") # FDR corrected for 902 comparisons

# Render the plot
print(plot)

# Save the plot to a specific folder as a PNG file
ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/INTERACTION_FDR_902_comparisons/",var,".png"), plot)
...
for (var in variables) {
  ...
  beta_sub_BL_centils[[var]] <- c(results_list2_BL_centils_sub[[var]]$values)
  beta_sub_time[[var]] <- c(results_list2_time_sub[[var]]$values)
  beta_sub_CPZ[[var]] <- c(results_list2_CPZ_sub[[var]]$values)
  beta_sub_interaction[[var]] <- c(results_list2_interaction_sub[[var]]$values)
  
  sig_sub_BL_centils[[var]] <- c(results_list2_BL_centils_sub[[var]]$sig)
  sig_sub_time[[var]] <- c(results_list2_time_sub[[var]]$sig)
  sig_sub_CPZ[[var]] <- c(results_list2_CPZ_sub[[var]]$sig)
  sig_sub_interaction[[var]] <- c(results_list2_interaction_sub[[var]]$sig)
  
  # Define the result types and their respective file names and titles
  result_types <- list(
    BL_centils = list(beta = beta_sub_BL_centils[[var]], sig = sig_sub_BL_centils[[var]], title = paste (var, "- Centils"), file_name = "BL_centils"),
    time = list(beta = beta_sub_time[[var]], sig = sig_sub_time[[var]], title = paste (var, "- Time"), file_name = "time"),
    CPZ = list(beta = beta_sub_CPZ[[var]], sig = sig_sub_CPZ[[var]], title = paste (var, "- CPZ"), file_name = "CPZ"),
    interaction = list(beta = beta_sub_interaction[[var]], sig = sig_sub_interaction[[var]], title = paste (var, "- Interaction"), file_name = "interaction")
  )
  ...
  for (result_key in names(result_types)) {
    ...
    if (endsWith(result$title, "Time") || endsWith(result$title, "CPZ")) {
      x_limits <- c(-20, 20)
      fill_limits <- c(-20, 20)
    } else {
      x_limits <- c(-5, 5)
    ...
    # Save the plot to a specific folder as a PNG file
    ggsave(paste0("/Users/manuelalejandromunozcaracuel/Documents/RIO HORTEGA/Neuroimagen/Images_symptoms/+SUBCORTICAL/",result$file_name,"/", var,".png"), plot, bg = "white")
  }
}
